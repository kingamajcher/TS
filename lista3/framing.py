...
def encode(input_file: str, output_file: str):
    ...
    with open(output_file, 'w') as file:
        while len(input_stream) > 0:
            ...
            bit_string = bit_string.replace('011111', '0111110')
...
def decode(input_file, output_file):
    ...
    with open(output_file, 'w') as file:
        for bit_string in input_stream:
            ...
            bit_string = bit_string.replace('01111110', '')
            bit_string = bit_string.replace('0111110', '011111')
            crc_string = bit_string[len(bit_string) - CRC_SIZE:]
            bit_string = bit_string[:len(bit_string) - CRC_SIZE]

            # verifying crc
            if calculator.verify(bytes(bit_string, 'utf-8'), int(bytes(crc_string, 'utf-8'), 2)):
                file.write(bit_string)
